...
.PARAMETER Server
    The Active Directory Domain Services instance to connect to. If not specified, the default domain controller for the current domain is used.

.PARAMETER Credential
    The credentials to use when connecting to Active Directory. If not specified, the current user's credentials are used.
...
.EXAMPLE
    ...
    This command retrieves a list of certificate templates published in Active Directory Certificate Services using the specified domain controller and credentials.

.DESCRIPTION
    This function retrieves a list of certificate templates published in Active Directory Certificate Services. Each output object lists a certificate template and a Boolean property for each enrollment server indicating whether that server has published the template. Template names are the template common names (CN), which may differ from their display names.

.INPUTS
    None. This function does not accept pipeline input.

.OUTPUTS
    PSCustomObject. One object per published certificate template. The 'Template' property contains the template common name (CN, not display name). One Boolean property per enrollment server (named by its DNS hostname) indicates whether that server publishes the template.
...
Function Get-PublishedCertificateTemplate {
    ...
    Catch {
        ...
        Write-Error -Message "The Active Directory PowerShell module is required to use this function. Please ensure the module is installed and available. Error: $($_.Exception.Message)" -ErrorAction Stop

    }

    # Build common parameters for Active Directory cmdlets
    $AdParams = @{}
    ...
    Try {
        ...
        $PkiContainerDN = "CN=Public Key Services,CN=Services,$($RootDSE.ConfigurationNamingContext)"
        ...
        $EnrollmentServers = @(Get-ADObject -Filter "objectClass -eq 'pKIEnrollmentService'" -SearchBase "CN=Enrollment Services,$PkiContainerDN" -SearchScope OneLevel -Properties DnsHostName, CertificateTemplates @AdParams -ErrorAction Stop | Where-Object DnsHostName | Sort-Object DnsHostName)
    ...
    # Exit if no enrollment servers were found
    If ($EnrollmentServers.Count -eq 0) {

        Write-Warning 'No AD CS enrollment servers were found.'
        Return
    ...
    $PublishedTemplates = @($EnrollmentServers.CertificateTemplates | Where-Object { $_ } | Sort-Object -Unique)

    # Exit if no published certificate templates were found
    If ($PublishedTemplates.Count -eq 0) {

        Write-Warning 'No published certificate templates were found.'
        Return

    }

    # Create a custom object for each published certificate template and output it to the pipeline
    ForEach ($Template in $PublishedTemplates) {

        $Row = [Ordered]@{ Template = $Template }

        ForEach ($CA in $EnrollmentServers) {

            $Row[$CA.DnsHostName] = [Bool]($CA.CertificateTemplates -contains $Template)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCRpuVcoJ+876/h
# GlH1SkKUaGoEriXNqd+nOilb5ssVj6CCH6YwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# DjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCDtQoG5eHd2uzuea/z8knYk
# +KK1HB3WOtRIoX2moxPosjALBgcqhkjOPQIBBQAERzBFAiA+hUWvg+zBmjzXyTo6
# r4fh6N4qjF3lve4QrFGcc0HIUgIhAMaij9UbsKPb199nGs9ZcVp6pGhQDMAOvEAI
# IWbpy3C8oYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMPAgEBMH0waTELMAkGA1UE
# BhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEwPwYDVQQDEzhEaWdpQ2Vy
# dCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2IFNIQTI1NiAyMDI1IENB
# MQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEFAKBpMBgGCSqGSIb3DQEJ
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDcyNzA0MDI1M1owLwYJ
# KoZIhvcNAQkEMSIEICG4X6HdV9wRTkY9zhdj/d/JGlwPWEYhhqid2jzVpB2sMA0G
# CSqGSIb3DQEBAQUABIICAMF4hoAeQSPjqJUCedtEyh+jbP+kGiYG2fAh9i0cVFCO
# +axAD+KYZoNvHhEWlGJpXd+L3482oY2+RDMODP+q3PIIP5PyNpEBbD+fz8OpIlJm
# DbIkqCTVVcI4L9SZhjqNvsbNzrewrqdM0dVO9TizjQFxJDYbz6gmCuu9Wp818gpd
# fbX5QbozwoniUuP+mQ54qu8SHJaBs+fxLwrPqx2Q1yTglJa+KUOUfu2fbmAwfrvA
# u+orCjKOrjnEq3/HHQarfrQrcxfvOx2R4cPg7Ji8Y/JKahAc+RUuSJMdAofmSldd
# qnTp8X1z091F7dpOHTQTXKca9/ljilN0OibZPSUa0TCI9YkRJhVGPGk/i5NMw8JY
# gdx2gsLHXGYzkJ3mkmXIKVk/yWOs1Dn4ccf+UdjU+SdLaMjmfRlDSxXKHGp10SXO
# HIoJIVodvTZZzQ/hPpIVEYY34ZeJuuQbhpIy159yTcOZrTUQhGVh4D5DQ8vMaBwQ
# gcISJPTiXawgwW191b4QiomYW0y3WWvWsQH80RX3ETku8c6TjN9d0cEYnS6JREaP
# x/cbyW5KDtQJegzGidGUkHn53upxLnXDd2EQQZlEB90O3nY7VtA7m7kn4rpGQhLT
# iXPPDUQs1i2ArwNXsduQdsaFXLrZQRK1NasKMxclWF0PLA2LaKeTFutW/0xO1P/k
